...
async function exportSessionToPDF(sessionData, nombreCompleto, fechaSesion) {
  ...
  logo.onload = function () {
    ...
    addContentToPDF(pdfDoc, sessionData, nombreCompleto, fechaSesion);
  };
  ...
  function addContentToPDF(pdfDoc, sessionData, nombreCompleto, fechaSesion) {
    // Configurar fuentes por defecto
    pdfDoc.setFont("helvetica", "normal");
    pdfDoc.setFontSize(10);
    ...
    // Insertar logo
    const logo = new Image();
    logo.src = "../img/logo.jpg"; // Ruta corregida al logo
    logo.onload = async function () {
      ...
      try {
        ...
        const userDocRef = firestoreDoc(db, "Simulador", user.uid);
        ...
        const fechaSesion =
          sessionData[0]?.["shot created date"]?.split(" ")[0] ||
        ...
        pdfDoc.text(
          nombreCompleto,
          pdfDoc.internal.pageSize.getWidth() - 10,
          15,
          {
            align: "right",
          }
        ...
        sessionData.forEach((shot) => {
          ...
          let cp = cs
            ? parseFloat(shot["club path (deg out-in-/in-out+)"])
            : "";
          ...
          clubsData[clubName].push({
            ...
            peakHeight: parseFloat(shot["peak height (yds)"]) * 3, // Convertir de yardas a pies
        ...
        descriptions.forEach((desc) => {
          ...
          // Usar splitTextToSize con la fuente por defecto
          ...
          // Verificar si necesitamos una nueva página basado en la altura del texto
